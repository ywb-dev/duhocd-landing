...
<template>
  <section id="university">
    <div class="relative w-full before:content-[''] before:absolute before:left-0 
      ...
      <div class="container max-w-960 px-9 pt-20 pb-24 overflow-hidden">
        ...
        class="flex flex-col justify-center items-center text-center">
          <img width="64" height="90" class="w-16 object-contain" src="/images/university/illustration_Achie_ement.png"
            loading="lazy" alt="thành tích">
          <h2
            class="text-shadow-white text-32 text-black text-center font-bold -tracking-[0.48px] leading-normal md:text-68 md:leading-[78px]">
            Các trường đại học</h2>
          <p class="text-shadow-white text-base max-w-2xl text-black font-normal leading-6 mt-3 mb-9">Hãy tham khảo danh
            sách các trường đại học do DUHOCD.com giới thiệu để bạn có thể tìm hiểu về ưu nhược điểm của mỗi trường. Từ đó
            xây dựng cho mình một kế hoạch du học phù hợp và hiệu quả nhất.</p>
        </div>
        <Transition>
          <div class="flex flex-col w-full">
            <div v-for="(inv, index) in unvs" :key="inv" :class="index === 0 ? '!border-[3px] active-first' : ''"
              ...
              <div ref="itemRefs" v-on:mouseover="mouseover(inv)" v-on:mouseleave="mouseleave(inv)"
                :class="index === 0 ? '!max-h-full' : ''" @click="toggleClick(index)"
                ...
                <div class="flex w-full md:w-1/2 rounded-[25px] overflow-hidden">
                  <img width="420" height="445" :class="inv.open ? 'block' : 'block'"
                    class="base-image max-h-[376px] md:max-h-full w-full object-cover" :src="inv?.img" loading="lazy"
                    alt="Đại học Hàn Quốc" />
                </div>
                <div class="unv-content mt-9 md:mt-0 flex flex-col w-full md:pl-8 md:w-1/2  overflow-y-auto no-scrollbar">
                  <h2 :class="{ active: index === 0 }"
                    class="heading content-white transition-all text-dark text-[27px] font-bold leading-normal mb-2 md:mt-12">
                    {{ inv?.title }}</h2>
                  <h3 class="content-white text-dark text-[19px] font-bold leading-normal mb-2 tracking-tight">{{
                    inv?.subtitle }}</h3>
                  <p :class="index === 0 ? '!opacity-100 !visible' : ''"
                    class="box-text-content pb-8 opacity-0 invisible transition-all duration-300 text-grey whitespace-pre-line text-15 font-normal leading-5 mt-5">
                    {{ inv?.content }}</p>
                </div>
              </div>
            </div>
...
.items:not(.active-first) .box-wrap-content:not(.clicked) .unv-content {
  @apply absolute h-full overflow-hidden bg-[#00000080] rounded-[16px] md:bg-white px-8 py-6 mt-0 md:py-0 md:static top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 md:translate-x-0 md:translate-y-0;
